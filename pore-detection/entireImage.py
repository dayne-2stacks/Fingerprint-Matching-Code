...
def findPores(
    ...
    if preDefinedPrediction == None:
        ...
        files = [os.path.isfile(imageFolder + "%s.jpg" % fileIndex) for fileIndex in index]
        if False in files: 
            return
    ...
    for fileIndex in (index):
        ...
        if preDefinedPrediction == None:
            ...
            if not os.path.isfile(imageFolder + "%s.jpg" % fileIndex): 
                break

            # read image and transoform it into proper tensor
            image = cv2.imread(imageFolder + "%s.jpg" % fileIndex, cv2.IMREAD_GRAYSCALE)
